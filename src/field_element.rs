    ...
    P: EmulatedFieldParams,
{
    ...
    pub fn conditionally_select2<CS: ConstraintSystem<F>>(
        cs: &mut CS,
        a0: &Self,
        a1: &Self,
        a2: &Self,
        ...
        if a0.len() != a1.len() || a1.len() != a2.len() || a2.len() != a3.len() {
            eprintln!("Current implementation of conditionally_select only allows same number of limbs");
            return Err(SynthesisError::Unsatisfiable);
        }
        let res_overflow = a0.overflow.max(a1.overflow).max(a2.overflow).max(a3.overflow);

        let a_values = [a0, a1, a2, a3].into_iter()
            .map(|a| {
                match &a.limbs {
        ...
        let res_values = match (condition1.get_value().unwrap(), condition0.get_value().unwrap()) {
            (false, false) => &a_values[0],
            (false,  true) => &a_values[1],
            (true,  false) => &a_values[2],
            (true,   true) => &a_values[3],
        };

        let res_alloc_limbs = EmulatedLimbs::allocate_limbs(
            &mut cs.namespace(|| "allocate conditional select2 result limbs"),
            res_values
        )?;

        let sub_vectors = |u: &Vec<F>, v: &Vec<F>| {
            let mut res_vec = vec![];
            for i in 0..u.len() {
                res_vec.push(u[i] - v[i]);
            }
            res_vec
        };
        let tmp1_value = if condition1.get_value().unwrap() {
            sub_vectors(&a_values[3], &a_values[2])
        } else {
            sub_vectors(&a_values[1], &a_values[0])
        };
        let tmp1 = EmulatedLimbs::allocate_limbs(
            &mut cs.namespace(|| "conditional select2 tmp1 value"),
            &tmp1_value,
        )?;

        let zero_value = vec![F::zero(); a0.len()];
        let tmp2_value = if condition0.get_value().unwrap() {
            tmp1_value
        } else {
            zero_value
        };
        let tmp2 = EmulatedLimbs::allocate_limbs(
        ...
        match (&res_alloc_limbs, &tmp1, &tmp2) {
            (
                EmulatedLimbs::Allocated(res_limbs),
                EmulatedLimbs::Allocated(tmp1_limbs),
                EmulatedLimbs::Allocated(tmp2_limbs)
            ) => {
                for i in 0..res_values.len() {
                    let a_lcs = [a0, a1, a2, a3].into_iter()
                        .map(|a| {
                            match &a.limbs {
                    ...
                    cs.enforce(
                        ...
                        |lc| lc + &a_lcs[3] - &a_lcs[2] - &a_lcs[1] + &a_lcs[0],
                        |_| condition1.lc(CS::one(), F::one()),
                        |lc| lc + &tmp1_limbs[i].lc(F::one()) - &a_lcs[1] + &a_lcs[0],
                    );
                    cs.enforce(
                        || format!("conditional select2 constraint 2 on limb {i}"),
                        |lc| lc + &tmp1_limbs[i].lc(F::one()),
                        |_| condition0.lc(CS::one(), F::one()),
                        |lc| lc + &tmp2_limbs[i].lc(F::one()),
                    );
                    cs.enforce(
                        || format!("conditional select2 constraint 3 on limb {i}"),
                        |lc| lc + &a_lcs[2] - &a_lcs[0],
                        |_| condition1.lc(CS::one(), F::one()),
                        |lc| lc + &res_limbs[i].lc(F::one()) - &tmp2_limbs[i].lc(F::one()) - &a_lcs[0],
                    );
                }
            },
...
mod tests {
    ...
    fn test_conditionally_select2() {
        ...
        for (c0, c1) in conditions.clone() {
            ...
            let res = EmulatedFieldElement::<Fp, Ed25519Fp>::conditionally_select2(
